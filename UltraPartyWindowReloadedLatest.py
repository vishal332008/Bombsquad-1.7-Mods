...
class PrivateChatHandler:
    ...
    def _cookie_login(self):
        self.myid = _ba.get_v1_account_misc_read_val_2('resolvedAccountID', '')
        try:
            with open(cookies_file, 'rb') as f:
                cookies = pickle.load(f)
        except:
...
class PartyWindow(ba.Window):
    ...
    def _send_chat_message(self) -> None:
        ...
        if not self._private_chat:
            if msg == '/id':
                myid = _ba.get_v1_account_misc_read_val_2('resolvedAccountID', '')
            ...
            name = _ba.get_v1_account_display_string()
            if not receiver:
...
class LoginWindow:
    def __init__(self, wtype):
        ...
        self._id = ba.textwidget(parent=self._root_widget,
                                 scale=0.5,
                                 color=(1, 1, 1),
                                 text=f'Account: ' + _ba.get_v1_account_misc_read_val_2('resolvedAccountID', ''),
                                 size=(0, 0),
                                 position=(220, 170),
                                 h_align='center',
                                 v_align='center')
...
original_sign_in = _ba.sign_in_v1
